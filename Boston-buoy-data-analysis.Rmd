...
Initially, we thought this would be the simplest process, since Professor Haviland had uploaded Web crawler script. However, due to different formats and different amount of variables, the code need to improve.

At first, we choose to separate the single `for` loop into two, one for downloading data from NOAA and one to deal with formats issues, which has two advantages. First, it is helpful to deal with network issues, especially when try to connect from China. No timeout will interrupt other process. Second, it is much more easier to compare difference of data from different years, which is necessary for further combination.

Nonetheless, using `read.table()` function for reading data from NOAA is not a good choice, because the NOAA data sets are not totally formal. Normally there is only 1 space between 2 columns, but in some cases 2 spaces will occur and the read.table() cannot handle such things. It will combine these columns separated by 2 spaces into 1 column as the outcome. Luckily, we can just use `read_table2()` function to solve this problem.

As a result, in this step, we will get 30 tables with total 246,245 observations and 16 to 18 variables depending on different years.
...
Last, in analysis, we need preset Y_W and Y_A to contain annual average WTMP and ATMP data. Also, we need preset M_W and M_A to contain monthly average WTMP and ATMP data.
...
As a common sense, the most direct evidence of global warming is that the temperature of the Earth has a upward trend. So, from all the variables listed above, only WTMP and ATMP are used. As for other wind or tide data their relations with global warming are not clear.
...
As the simple linear regression model shows the slope of fitted line is 0.005413 which represents a slightly upward trend of buoy water temperature indicating global warming and can be interpreted as buoy water temperature will increase by 0.005413 $^{\circ}C$ per year. The picture also shows the same result.

### Trends of monthly average water temperature

Now we found some evidence, but the slope or annual increase of 0.005413 $^{\circ}C$ is so samll that is hard to convince us. We need to separate this slope so that may be more information will be reveal. And we choose to further analyze the data from a month perspective.
...
As the simple linear regression model shows the slope of fitted line is 0.003434 which also represents a slightly upward trend of buoy air temperature indicating global warming and can also be interpreted as buoy air temperature will increase by 0.003434 $^{\circ}C$ per year. The picture no doubtly shows the same result.
